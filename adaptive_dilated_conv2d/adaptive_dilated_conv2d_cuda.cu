...
#include "adaptive_dilated_conv2d.h"
...
__global__ void adaptive_dilated_conv2d_im2col_kernel(
    int n,
    const float* data_im,
...
__global__ void adaptive_dilated_conv2d_col2im_coord_kernel(
 const int n,
...
__global__ void adaptive_dilated_conv2d_col2im_kernel(
    const int n,
    const float* data_col,
    const float* data_rate,
...
void adaptive_dilated_conv2d_im2col(cudaStream_t stream,
    const float* data_im,
    const float* data_rate,
...
//     const int dilation_h, const int dilation_w,
    ...
    adaptive_dilated_conv2d_im2col_kernel<<<GET_BLOCKS(num_kernels), CUDA_NUM_THREADS, 0, stream>>>(
            num_kernels,
            data_im,
            data_rate,
            height, width,
...
void adaptive_dilated_conv2d_col2im_coord(cudaStream_t stream,
   const float* data_col, const float* data_im, const float* data_rate,
   const int in_channels, const int height, const int width, // 输入的C, H, W
   const int kernel_h, const int kernel_w,
   const int stride_h, const int stride_w,
   const int height_col, const int width_col,
   float* grad_rate_map){
   int num_kernels = in_channels * kernel_h * kernel_w * height_col * width_col;
   adaptive_dilated_conv2d_col2im_coord_kernel<<<GET_BLOCKS(num_kernels), CUDA_NUM_THREADS, 0, stream>>>(
       num_kernels,
       data_col,
       data_im,
       data_rate,
...
void adaptive_dilated_conv2d_col2im(cudaStream_t stream,
    ...
    adaptive_dilated_conv2d_col2im_kernel<<<GET_BLOCKS(num_kernels), CUDA_NUM_THREADS, 0, stream>>>(
        num_kernels,
        data_col,
        data_rate,
        in_channels, height, width,
