...
at::Tensor adaptive_dilated_conv2d_forward(
    at::Tensor input,
    at::Tensor weight,
    at::Tensor rate_map,
    at::Tensor bias,
...
//     int kernel_channels = weight.size(1);
    ...
    **/
    ...
    for (int n = 0; n < batch; ++n) {
        adaptive_dilated_conv2d_im2col(
...
std::vector<at::Tensor> adaptive_dilated_conv2d_backward(
    at::Tensor input,
    at::Tensor weight,
    at::Tensor rate_map,
    at::Tensor bias,
...
//     auto bias_ptr = bias.data<float>();
    ...
    for (int n = 0; n < batch; ++n) {
        ...
        **/
       adaptive_dilated_conv2d_col2im_coord(
           THCState_getCurrentStream(state),
           col_buffer_ptr,
           input_ptr + n * input_dim,
           rate_map_ptr + n * conv_out_spatial_dim,
        ...
        **/
        adaptive_dilated_conv2d_col2im(
            THCState_getCurrentStream(state),
            col_buffer_ptr,
            rate_map_ptr + n * conv_out_spatial_dim,
            in_channels, input_height, input_width,
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m){
  m.def("forward", &adaptive_dilated_conv2d_forward, "adaptive dilated conv2d forward (CUDA)");
  m.def("backward", &adaptive_dilated_conv2d_backward, "adaptive dilated conv2d backward (CUDA)");
}
