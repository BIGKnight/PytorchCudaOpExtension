...
__global__ void DeformableConv2DCol2ImKernel(
    ...
    CUDA_KERNEL_LOOP(index, n){
        ...
        const int c = index / width_col / height_col / batch_size / kernel_w / kernel_h; // which channel
        ...
        //这里写法很奇怪, 我觉得只需要改四个点, (0, 0), (0, 1), (1, 0), (1, 1), 但是他这里有25个点....啥玩意...而且第一个条件语句其实限制了
        //cur_h不是肯定小于cur_inv_h_data吗?
        //abs(cur_inv_h_data - (cur_h + dy)) < 1 这个条件设着, 怎么可能还会有dy = -2, -1这种可能性
